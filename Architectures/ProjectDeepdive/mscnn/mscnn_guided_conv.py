...
import Architectures.Layers.guided_conv as gc

class MscnnGuidedConv(architecture.Architecture):
    def __init__(self):
    ...
    def prediction(self, sample, training=False):
        ...
        # conv1 = gc.guidedfilter_conv2d(inputs=sample,num_outputs=5,guide_kernel_size = [3, 3],
        #                                 guided_kernel_size=[7,7], stride=[1, 1], padding='SAME')
        ...
        linear_combination2 = tf.contrib.layers.conv2d(inputs=upsamp6, num_outputs=1,
                                                       ...
                                                       normalizer_params=normalizer_params,
                                                       activation_fn=tf.nn.sigmoid)
        guided_conv = gc.guidedfilter_conv2d(guided_inputs=linear_combination2, guide_inputs=sample, 
                                       num_outputs=1,guide_kernel_size = [11, 11],
                                       guided_kernel_size=[11,11], stride=[1, 1], padding='SAME')
        relu = tf.nn.relu(guided_conv)
        tf.summary.image("architecture_output", relu)
        return relu
